...
public class GestioneCampeggio {
	...
	public Prenotazione EffettuaPrenotazione(LocalDate DataInizio, LocalDate DataFine, int NomeSettore,
			String Categoria, String tipo, String Email) throws OperationException {
		...
		try {
			...
			if (settore == null) {
				throw new OperationException("Settore non trovato");
			}
			...
			if (PiazzoleDisponibili == null) {
				throw new OperationException("Piazzola non disponibile");
			}
			...
			if (cliente == null) {
				throw new OperationException("Cliente non registrato!!");
			}
	 ...
	 * System.out.println("Prenotazione annullata.\n"); //il rifferimento verrà
	 * eliminato dal garbage collector in quanto sarà inutilizzata o sarà
	 * riutilizzata in altre prenotazioni (overridden) }
	 */
	...
	public String AcquistaServizio(int CodiceConto, int IdServizio) throws OperationException {
		...
		try {
			...
			if (conto == null) {
				throw new OperationException("Conto non presente");
			}

			if (!(conto.getStato().equals("ATTIVO"))) {
				throw new OperationException("Conto non attivo");
			}

			if (servizio == null) {
				throw new OperationException("Servizio inesistente");
			}

			// this.conto = new ContoSpese(conto.getCodiceConto(), conto.getStato(),
			// conto.getTotaleCorrente(), conto.getClienteRegistrato());
			// this.servizio = new Servizio (servizio.getPrezzoServizio(),
			// servizio.getTipoServizio(), servizio.getIdServizio());

			acquisto = new Acquisto(CodiceConto, IdServizio);
			AcquistoDAO.createAcquisto(acquisto);
			// aggiornaConto(this.conto, this.servizio);

			return "Utilizzo servizio confermato";
		} catch (DBConnectionException dEx) {
			throw new OperationException("Riscontrato errore nell'acquisto. Problema interno!");
		} catch (DAOException ex) {
	...
	/*
	 * private void aggiornaConto (ContoSpese c, Servizio s) throws
	 * OperationException { ContoSpese conto = c; Servizio servizio = s; float
	 * prezzo = 0; try { prezzo = servizio.getPrezzoServizio() +
	 * conto.getTotaleCorrente(); conto.setTotaleCorrente(prezzo);
	 * ContiSpeseDAO.updateTotaleContoSpesa(conto, conto.getCodiceConto());
	 * }catch(DBConnectionException dEx){ throw new
	 * OperationException("\nRiscontrato problema interno aggiorno conto spese!\n");
	 * }catch(DAOException ex) { throw new
	 * OperationException("Ops, errore conto non aggiornato!\n"); }
	 * 
	 * }
	 */

	public void invioSpeseEmail(int codice) throws OperationException {
	    String email=null;
	    float totale;
	    ArrayList<Servizio> ElencoSpese = null;
	    try {
	      conto = ContiSpeseDAO.readContoSpese(codice);
	      ...
	      totale=ContiSpeseDAO.getTotaleCorrente(conto.getCodiceConto());
	...
	public void RegistrazionePagamento(int CodiceConto) throws OperationException {
		...
		try {
			...
			if (totale == 0) {
				throw new OperationException("Conto vuoto!!\n");
			}
			// this.conto = new ContoSpese(conto.getStato(), conto.getTotaleCorrente(),
			// conto.getClienteRegistrato());
			//stampaContoFinale(conto, totale);
			this.ChiusuraConto(conto);
			this.InvioContoFinale(conto);
		} catch (DBConnectionException dEx) {
	 ...
	 * email=this.conto.getClienteRegistrato(); InvioContoFinale(email, totale);
	 * conferma = ChiusuraConto(this.conto); return conferma;
	 * }catch(DBConnectionException dEx){ throw new
...
*/
	private void InvioContoFinale(ContoSpese conto) throws OperationException {
		...
		try {
			email = new String(conto.getClienteRegistrato());
			ElencoSpese = new ArrayList<Servizio>(AcquistoDAO.getListaServizi(conto.getCodiceConto())); // aggiungere al
																										// datbase
	...
	public int AperturaConto(String Email) throws OperationException {
		...
		try {
			cliente = ClienteRegistratoDAO.readClienteRegistrato(Email);
			if (cliente == null) {
				throw new OperationException("Cliente non presente!!\n");
			}
	...
	public String Registrazione(String Nome, String Cognome, String Email, String Username, String Password,
			int Telefono) throws OperationException {
	...
	public void VisualizzaDisponibilità(LocalDate DataInizio, LocalDate DataFine, int NomeSettore, String Categoria,
			String Tipo) throws OperationException {
		ArrayList<Piazzola> PiazzoleDisponibili = null;
		Settore settore = null;
		try {
			settore = SettoreDAO.readSettore(NomeSettore);
